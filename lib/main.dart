...
//           pushing /reset-password. The previous approach raced with app
//           initialization and fell through to onUnknownRoute ("Page Not Found").
...
class _MyAppState extends State<MyApp> {
  ...
  /// Push the reset password page using a post-frame callback to guarantee
  /// the navigator is mounted and ready — this prevents the race condition
  /// that caused "Page Not Found" in the previous implementation.
  void _navigateToReset(Session session) {
    AppConfig.debugPrint('🔀 Scheduling navigation to /reset-password...');
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (nav != null) {
        ...
        AppConfig.debugPrint('⚠️ Navigator still null — retrying in 300ms');
  ...
  /// Fallback deep link handler for cases where onAuthStateChange doesn't
  /// fire automatically (some Android custom-scheme edge cases).
  Future<void> _handleDeepLink(Uri uri) async {
    ...
    if (uriStr.contains('reset-password') ||
      ...
      AppConfig.debugPrint('🔑 Reset-password deep link — parsing session...');
      ...
      try {
        ...
        if (response.session != null) {
          AppConfig.debugPrint('✅ Session parsed from URL (fallback path)');
          _navigateToReset(response.session!);
        } else {
          AppConfig.debugPrint('⚠️ No session in URL — showing expired link');
          _navigateToExpiredLink();
        }
      } catch (e) {
        AppConfig.debugPrint('⚠️ getSessionFromUrl failed: $e');
        // Don't show expired yet — onAuthStateChange may still fire
      }
    }
  }
  ...
  void _navigateToExpiredLink() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _navigatorKey.currentState?.pushNamedAndRemoveUntil(
        '/reset-password',
        (route) => false,
        arguments: null, // null session → ResetPasswordPage shows expired UI
      );
    });
  }
